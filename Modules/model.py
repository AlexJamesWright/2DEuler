...
class TwoFluidEMHD(object):
    def __init__(self, grid, g=5/3, mu1=-1e4, mu2=1e4, sig=1e3, kappa=1):
        ...
        mu2 : float
            Charge mass ratio of species two
        sig : float
            Conductivity (reciprocal of resistivity)
        kappa : float
            Constant for divergence cleaning, speed at which phi and psi are
            suppressed

        Notes
        -----
            We evolve the following conserved vector:
    ...
    def fluxFunc(self, q, simulation, direction):
        ...
        ----------
            q : array of float
                (Nvars, Nx, Ny) The conserved vector
            sim : object
                Overarching simulation class
            direction : int
                Direction of intended flux. x = 0, y = 1.
        
        Returns
        -------
            flux : array of float
                Reconstructed flux in specified  direction at the cell faces.
        """
        # Short hand
        mu1 = self.mu1
        mu2 = self.mu2
    ...
    def getPrimitiveVars(self, q, sim):
        ...
        -------
            prims : array of float
                (Nprims, Nx, Ny) Primitive vector corresponding to the conserved 
                vector
            aux : array of float
                (Naux, Nx, Ny) Auxilliary vector corresponding to the conserved
                vector
            alpha : float
                Max wave speed
        """
